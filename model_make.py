...
#画像の格納
path_BI = [filename for filename in os.listdir('/content/drive/MyDrive/ブルーアイズ') if not filename.startswith('.')] 
path_BM = [filename for filename in os.listdir('/content/drive/MyDrive/ブラックマジシャン') if not filename.startswith('.')]
path_RI = [filename for filename in os.listdir('/content/drive/MyDrive/レッドアイズ') if not filename.startswith('.')]
path_GIA =[filename for filename in os.listdir('/content/drive/MyDrive/暗黒騎士ガイア') if not filename.startswith('.')]
path_MOS = [filename for filename in os.listdir('/content/drive/MyDrive/究極完全態・グレート・モス') if not filename.startswith('.')]
...
for i in range(len(path_BI)):
    img = cv2.imread('/content/drive/MyDrive/ブルーアイズ/' + path_BI[i])
    img = cv2.resize(img,(image_size, image_size))
    img_BI.append(img)

for i in range(len(path_BM)):
    img = cv2.imread('/content/drive/MyDrive/ブラックマジシャン/' + path_BM[i])
    img = cv2.resize(img,(image_size, image_size))
    img_BM.append(img)

for i in range(len(path_RI)):
    img = cv2.imread('/content/drive/MyDrive/レッドアイズ/' + path_RI[i])
    img = cv2.resize(img,(image_size, image_size))
    img_RI.append(img)

for i in range(len(path_GIA)):
    img = cv2.imread('/content/drive/MyDrive/暗黒騎士ガイア/' + path_GIA[i])
    img = cv2.resize(img,(image_size, image_size))
    img_GIA.append(img)

for i in range(len(path_MOS)):
    img = cv2.imread('/content/drive/MyDrive/究極完全態・グレート・モス/' + path_MOS[i])
    img = cv2.resize(img,(image_size, image_size))
    img_MOS.append(img)
